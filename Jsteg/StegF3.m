...
while index<=length(plain)
    ...
    % if plain=1 and even->abs-1
    % if plain=0 and odd->abs-1
    % if plain=1 and odd->Keep
    % if plain=0 and even->Keep
    % if plain=0 and 1/-1->Pass
    if plain(index)=='1'
        if mod(activedobj.coef_arrays{1,1}(row,column),2)==0
            if activedobj.coef_arrays{1,1}(row,column)>0
        ...
        if activedobj.coef_arrays{1,1}(row,column)==1||activedobj.coef_arrays{1,1}(row,column)==-1
            %Pass
            %Shrink to 0
            activedobj.coef_arrays{1,1}(row,column)=0;
             RandTableIndex=RandTableIndex+1;
             fprintf("(1/-1,0),Fail to fatch,%d\t%d\n",RandTableIndex,randtable(RandTableIndex));
             continue;
        end
        if  mod(activedobj.coef_arrays{1,1}(row,column),2)==1
            if activedobj.coef_arrays{1,1}(row,column)>0
                activedobj.coef_arrays{1,1}(row,column)=activedobj.coef_arrays{1,1}(row,column)-1;
            else
                activedobj.coef_arrays{1,1}(row,column)=activedobj.coef_arrays{1,1}(row,column)+1;
...
jpeg_write(activedobj,'kinkakuji-F3.jpg');
%Show DCT coefficient histogram
Aft=imread('kinkakuji-F3.jpg');
subplot(2,2,3)
imhist(I);
...
subplot(2,2,4)
imhist(Aft);
title("DCT coefficient histogram-F3");
